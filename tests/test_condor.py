...
def test_open_trade_saves_user_defined_kwargs(make_condor):
    c = make_condor(fill_factor=1.0)
    c.open_trade(quantity=1, tag='test')
...
def test_close_trade_closes_all_legs(make_condor):
    c = make_condor(fill_factor=1.0)
    c.open_trade()
    c.close_trade(quote_datetime=c.lower_option.quote_datetime)
...
def test_close_trade_requires_keyword_only_quote_datetime(make_condor):
    c = make_condor(fill_factor=1.0)
    c.open_trade()
    c.close_trade(quote_datetime=c.lower_option.quote_datetime)  # must not raise
...
def test_price_history_pnl_is_zero_at_open_bar(make_condor):
    c = make_condor(fill_factor=1.0)
    c.open_trade()
    assert c.get_price_history()[0]['pnl'] == pytest.approx(0.0, abs=1.0)

def test_price_history_pnl_pct_is_zero_at_open_bar(make_condor):
    c = make_condor(fill_factor=1.0)
    c.open_trade()
    assert c.get_price_history()[0]['pnl_pct'] == pytest.approx(0.0, abs=0.01)
