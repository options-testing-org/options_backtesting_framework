...
@pytest.mark.parametrize("option_type, strike, quantity, expected_repr", [('call', 110.0, 1, '<SINGLE({0}) AAPL CALL 110.0 2015-01-17{1}>'),
                                                                          ('call', 120.0, -1, '<SINGLE({0}) AAPL CALL 120.0 2015-01-17{1}>'),
                                                                          ('put', 130.0, 1, '<SINGLE({0}) AAPL PUT 130.0 2015-01-17{1}>'),
                                                                          ('put', 100.0, -1, '<SINGLE({0}) AAPL PUT 100.0 2015-01-17{1}>')
                                                                          ])
...
def test_naked_put_margin_requirement_otm(option_chain_data):
    ...
    single_option = Single.create(option_chain=option_chain, expiration=expiration, strike=strike, option_type='put')
    single_option.open_trade(quantity=-1)

    assert single_option.required_margin ==  1_142.2
    assert single_option.position_type == OptionPositionType.SHORT
...
def test_get_single_option_gets_next_expiration_when_expiration_is_not_in_chain(option_chain_data):
    ...
    single_option = Single.create(option_chain=option_chain, expiration=expiration, strike=strike, option_type='call')
    single_option.open_trade(quantity=qty)
...
def test_open_trade_sets_quantity_and_status(option_chain_data):
    ...
    single = Single.create(option_chain=option_chain, expiration=expiration, strike=strike, option_type='call')
