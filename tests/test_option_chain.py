...
def db_data_loader():
    pass
    # original_value_3 = settings.DATA_FORMAT_SETTINGS    settings.DATA_FORMAT_SETTINGS = "sql_server_cboe_settings.toml"
    # start = datetime.datetime(2016, 3, 1, 9, 31)
    # end = start = datetime.datetime(2016, 3, 1, 9, 32)
    # select_filter = SelectFilter(option_type=OptionType.CALL)
    # data_loader = SQLServerDataLoader(start=start, end=end, select_filter=select_filter)
    #
    # return data_loader, start, end
...
def test_load_option_chain_from_db(db_data_loader):
    ...
    # data_loader.load_cache('SPXW', quote_datetime)
