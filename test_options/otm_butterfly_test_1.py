...
class OTMButterflyStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if t == self.p.market_open:
            ...
            expiration=self.option_chain.expirations[0]
            ...
            for idx in range(start_idx, len(strikes)):
                ...
                butterfly = Butterfly.get_position(options=self.option_chain.option_chain,
                                      ...
                                      center_strike=center_strike, wing_width=20, quantity=1)
        ...
        if current_position:
            ...
            percent_of_high = highest_profit/current_profit
            pl = current_position.current_value / current_position.max_loss
            ...
            if t_slot == TimeSlot.MIDDLE:
                if zone == Zone.ZONE_1 and pl <= 0.4:
                    close_position = True
                if zone == Zone.ZONE_2 and percent_of_high <= 0.30:
                    close_position = True
            ...
            if close_position:
                ...
                print(f'close position {current_position.position_id} @ {t} {current_position.current_value - current_position.max_loss}')
                current_position = None

        if t == self.p.end_afternoon and current_position:
            self.portfolio.close_position(current_position, quantity=current_position.quantity)
        if t == self.p.eod:
            self.log(
